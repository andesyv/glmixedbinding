...
static const auto noneVersion = glmixedbinding::Version(0, 0);
static const auto noneStringSet = std::set<std::string>{};
static const auto noneExtensions = std::set<glmixed::GLextension>{};
static const auto noneVersions = std::set<glmixedbinding::Version>{};
...
int Meta::glmixedRevision()
{
	// Since glmixed.xml moved to github repository, the revision is currently not available and always returns NULL.
    return GLMIXED_REVISION;
}

size_t Meta::alphabeticalGroupIndex(const std::string & identifier, const std::uint8_t prefixLength)
{
    auto index = static_cast<size_t>(identifier[prefixLength]); // ignore prefix ('GL_' or 'glmixed')
...
glmixed::GLboolean Meta::getBoolean(const std::string & glboolean)
{
